...
|---|---|---|---|
| **Quests** (story / side / bounty / guild / arena) | `parsed.json` `quests` — 185 entries | `quests[]` in `POST /quests` | none; accepted per character |
...
Coverage: **128 of the 185** quests in `parsed.json` have a flat reward backed by
captures or the matching APK reward preview. Adding the 39 event templates (below)
brings the total to **167 / 185 (90 %)**.

The other 18 have no non-zero flat payout or event ladder. **They pay nothing and log

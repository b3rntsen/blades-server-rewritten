...
| **Level-up** | `POST /levelup` | ✅ Done | Spends `level_rewards[newLevel].xp_to_reach` and refuses an unearned level; answers with `wallet` only when the level credited a currency. The curve WAS captured — measured over 37 retail level-ups, `docs/retail-journey.md` §2. |
...
| **Visiting another town's shops** | `POST …/social/users/{u}/characters/{c}/shops/{s}`(+`/purchase`) | ✅ Done | The owner's stock and ledger, the visitor's gold and goods, one transaction. 1,673 captures from 9 players; there was no route at all until the endpoint-coverage check found it. |
...
| **Town building** | `POST /towns/current/buildings`(+`/{id}/{upgrade,complete,destroy,styles/{id}}`,`/props`,`/name`) | ✅ Done | Place / upgrade / complete / destroy / restyle / props / name, charged from `building_upgrades.json` with real construction timers and gem speed-ups. Town XP is paid on **completion** and on a **restyle**, and on nothing else — measured over 357 captured mutations, `docs/retail-journey.md` §2. |
